...
import {
  GENESIS_AXIS_PHENOMENON_LABEL_EN,
  GENESIS_AXIS_PHENOMENON_LABEL_KO,
  GENESIS_AXIS_PRACTICE_LABEL_EN,
  GENESIS_AXIS_PRACTICE_LABEL_KO,
  GENESIS_ELEVEN_AXES,
  GENESIS_ELEVEN_AXES_SECTION_LEAD_EN,
  GENESIS_ELEVEN_AXES_SECTION_LEAD_KO,
  GENESIS_ELEVEN_AXES_SECTION_TITLE_EN,
  GENESIS_ELEVEN_AXES_SECTION_TITLE_KO,
} from "@/lib/lumina/lumina-genesis-eleven-axes-copy"
...
export function GenesisElevenAxesTree() {
  ...
  return (
    <section
      id="genesis-eleven-axes"
      aria-labelledby={`${uid}-genesis-axes-title`}
      className="mt-16 scroll-mt-28 border-t border-white/[0.08] pt-14 sm:mt-20 sm:pt-16"
    >
      <div className="mb-10 max-w-2xl">
        <h2
          id={`${uid}-genesis-axes-title`}
          className={cn(
            "text-[1.15rem] font-semibold tracking-[0.02em] text-[#eef2f8] sm:text-[1.25rem]",
            isKo ? fontSerifKo : fontSerifEn,
          )}
        >
          {isKo ? GENESIS_ELEVEN_AXES_SECTION_TITLE_KO : GENESIS_ELEVEN_AXES_SECTION_TITLE_EN}
        </h2>
        <p className={cn("mt-3 text-[0.875rem] leading-[1.75] text-[#9eb0c4]", fontSans)}>
          {isKo ? GENESIS_ELEVEN_AXES_SECTION_LEAD_KO : GENESIS_ELEVEN_AXES_SECTION_LEAD_EN}
        </p>
      </div>

      <ol className="grid grid-cols-1 gap-3 md:grid-cols-2 md:gap-4">
        {GENESIS_ELEVEN_AXES.map((axis, index) => {
          const isOpen = !!open[axis.id]
          const phenomenonLabel = isKo
            ? GENESIS_AXIS_PHENOMENON_LABEL_KO
            : GENESIS_AXIS_PHENOMENON_LABEL_EN
          const practiceLabel = isKo ? GENESIS_AXIS_PRACTICE_LABEL_KO : GENESIS_AXIS_PRACTICE_LABEL_EN
            ...
            <li key={axis.id} className="list-none">
              ...
              >
                <button
                  ...
                  aria-controls={`${uid}-panel-${axis.id}`}
                  id={`${uid}-btn-${axis.id}`}
                  className={cn(
                ...
                >
                  <span className="min-w-0 flex-1">
                    ...
                    >
                      {isKo ? axis.metaphorKo : axis.metaphorEn}
                    </span>
                  </span>
                  <ChevronDown
                    className={cn(
                ...
                <AnimatePresence initial={false}>
                  {isOpen ? (
                    <motion.div
                      id={`${uid}-panel-${axis.id}`}
                      role="region"
                      aria-labelledby={`${uid}-btn-${axis.id}`}
                      ...
                      <div className="space-y-0 px-4 pb-4 pt-1 sm:px-5">
                        <div className="relative border-l-2 border-[#c9a227]/45 py-2 pl-4">
                          ...
                          <p className={cn("text-[0.8125rem] leading-[1.78] text-[#dce4ee] sm:text-[0.875rem]", fontSans)}>
                            {isKo ? axis.phenomenonKo : axis.phenomenonEn}
                          </p>
                        </div>
                        ...
                        <div className="mt-3 rounded-sm border border-[#d4af37]/15 bg-[rgba(212,175,55,0.04)] px-3 py-3 sm:px-4">
                          ...
                          <p className={cn("text-[0.8125rem] leading-[1.78] text-[#b8c4d4] sm:text-[0.875rem]", fontSans)}>
                            {isKo ? axis.practiceKo : axis.practiceEn}
                          </p>
                        </div>
                      </div>
                    </motion.div>
